    ...
    <mu-paper class="item_box" :z-depth="1" v-for="(item,index) in myList" :key="'item'+index">
      ...
      <div class="lease_content">
        <div class="left_text">
          ...
          <div v-if="item.showType===1">
            <div class="item">
              ...
              <span class="rightText v_ellipsis">{{item.bond}}元</span>
            </div>

            <div class="item">
              <span class="leftText v_ellipsis">预定时间</span>
              <span
                v-if="item.status!=11&&item.status!=12"
                class="rightText v_ellipsis"
              >{{item.begin_date+'/'+(item.end_date?item.end_date:'--')}}</span>
              <span
                v-if="item.status===11||item.status===12"
                class="rightText v_ellipsis"
              >{{item.begin_date+'--'}}</span>
            </div>
            <div class="item" v-if="item.status!=11&&item.status!=12">
              <span class="leftText v_ellipsis">距离失效</span>
              <span class="rightText v_ellipsis red">{{residueTime(item.end_date)}}</span>
            </div>
          </div>
          <!-- 办理中 -  在租中 -->
          <div v-if="item.showType===2">
            ...
            <div class="item">
              ...
              <span class="rightText v_ellipsis">{{setPayModelName(item.pay_mode)}}</span>
            </div>
            ...
            <div class="item" v-if="item.status===1">
              <span class="leftText v_ellipsis">下次交租</span>
              <span class="rightText v_ellipsis">{{item.next_pay_date}}</span>
            </div>
          </div>
          <!-- 退租 -->
          <div v-if="item.showType===3">
            <div class="item">
              ...
              <span class="rightText v_ellipsis">{{item.rent}}</span>
            </div>
            <div class="item">
          ...
          <div class="item">
            ...
            <span class="rightText v_ellipsis" :style="{color:item.bg}">{{item.saler}}</span>
          </div>
          <!-- 状态 -->
          <div class="item">
        ...
        <div class="right_btn">
          <div
            class="btn_border"
            v-if="setBtnShow(item.status)"
            @click="toLeaseBill(item.lease_id,item.electFlag)"
          >
            <mu-button class="btn" color="#C6A700" flat>账单</mu-button>
          </div>
          <div class="btn_border" v-if="setBtnShow(item.status)" @click="examineContract(item)">
            <mu-button class="btn" color="#C6A700" flat>电子合同</mu-button>
          </div>
          ...
          <mu-button
            ...
            color="#545454"
            textColor="#ffffff"
            @click="cancleReserve(item.lease_id,index)"
            v-if="(item.status===9||item.status===11)&&item.owen"
            round
          >删除预定</mu-button>
          <mu-button
            class="btn"
            color="#545454"
            textColor="#ffffff"
            @click="deleteOrder(item.lease_id,index)"
      ...
      <div class="footer_tag">
        <span class="tag_item" v-if="item.owen">我的签约</span>
        <span class="tag_item" v-if="item.duty">责任租约</span>
        <!--electFlag 0 --纸质合同 1电子合同-->
        <span class="tag_item" v-if="item.electFlag===0">老租客</span>

        <!--extendFlag 0 --新签 1续签-->
        <span class="tag_item" v-if="item.extendFlag===0">续租合同</span>
      </div>
    </mu-paper>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    // 办理中租约删除
    deleteOrder (leaseid, index) {
      this.$confirm('确认将租约作废？', '提示').then(val => {
        if (val.result) {
          cancelMyOrder(leaseid).then(res => {
            this.$emit('changeList', index);
          });
        }
      });
    ...
    setItemData (arr) {
      ...
      arr.forEach(item => {
        ...
        item.duty = this.showDutyTag(item.roomInfo.house_info.sale_id); // 责任销售
        item.owen = this.showSelfTag(item.saler_id); // 我的签约
        switch (item.status) {
          case 4: // 解约 紫色
          case 6: // 驳回
            item.bg = '#B383E2';
