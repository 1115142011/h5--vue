<template>
  <section class="v_page" @click="searchState = false">
    <mu-paper class="v_topNav topNav" :z-depth="4">
      <!-- 顶部导航 -->
      <mu-appbar color="#fdd835" title="退租/驳回" textColor="#000">
        <!-- 导航图标 -->
        <mu-button icon slot="left" @click="goBack" v-show="!searchState">
          <mu-icon value="apps"></mu-icon>
        </mu-button>
        <div slot="right" class="right_solt" v-show="!searchState">
          <mu-button icon @click.stop="showBox">
            <mu-icon value="account_box"></mu-icon>
          </mu-button>
          <mu-button icon @click.stop="showBox">
            <mu-icon value="search"></mu-icon>
          </mu-button>
        </div>
        <!-- 搜索框 -->
        <div class="search_wrap" slot="default" v-if="searchState">
          <div class="searchBox skipRem" ref="searchBox" @click.stop>
            <mu-text-field
              v-model="params.queryTerm"
              placeholder="房源/租客姓名"
              solo
              action-icon="cancel"
              :action-click="clearInput"
              :full-width="true"
            ></mu-text-field>
          </div>
          <mu-button class="cancle skipRem" flat @click="cancleSearch">取消</mu-button>
        </div>
      </mu-appbar>
      <!-- tab选项  卡 -->
      <mu-tabs
        color="#fdd835"
        :value.sync="params.status"
        indicator-color="#448AFF"
        full-width
        center
      >
        <mu-tab v-for="(item,index) in tabs" :key="index" :value="item.value" class="v_ellipsis">
          <div class="tabs_item">
            <!-- <span class="sup" v-if="index!=0">{{item.total}}</span> -->
            {{item.name}}
          </div>
        </mu-tab>
      </mu-tabs>
    </mu-paper>
    <!-- 列表 -->
    <div class="v_mainContainer skipRem" ref="container">
      <mu-load-more
        @refresh="refresh"
        :refreshing="refreshing"
        :loading="loading"
        @load="load"
        :loaded-all="loadAll"
      >
        <list-item :listData="listData"></list-item>
      </mu-load-more>
      <div class="v_footerHint" v-show="loadAll">没有更多数据了~</div>
    </div>
  </section>
</template>
<script>
import { queryRejectLease } from '@/api/salesApi';
import ListItem from './child/ListItem';
export default {
  components: { ListItem },
  data () {
    return {
      time_out: null,
      listData: [],
      loadAll: false,
      refreshing: false,
      loading: false,
      searchState: false,
      active: 0, // tab 卡选中项
      params: {
        queryTerm: undefined, // 收索关键词参数
        status: 3, // 房间状态--tab 选中项参数
        pageSize: 5
      },
      pageIndex: 1,
      // tab 卡选项
      tabs: [
        {
          name: '退租驳回',
          value: 3,
          total: 1
        },
        {
          name: '审核中',
          value: 1
        },
        {
          name: '结算中',
          value: 2
        }
        // {
        //   name: '全部',
        //   value: 0
        // }

        // {
        //   name: '退租中',
        //   value: 4,
        //   total: 15
        // }
      ]
    };
  },
  methods: {
    // 返回
    goBack () {
      this.$store.commit('change_Action', 'pop');
      this.$router.go(-1);
    },
    // 点击搜素按钮
    showBox () {
      this.searchState = true;
    },
    // 清除输入
    clearInput () {
      this.params.queryTerm = '';
    },
    // 取消收索
    cancleSearch () {
      this.params.queryTerm = '';
      this.searchState = false;
    },
    // 加载更新
    refresh () {
      this.refreshing = true;
      this.loadAll = false;
      this.listData = [];
      this.$refs.container.scrollTop = 0;
      this.pageIndex = 1;
      this.getListData(this.params, this.pageIndex);
    },
    // 加载列表数据api
    load () {
      this.getListData(this.params, this.pageIndex);
    },
    // 加载 列表
    getListData (pram, index) {
      let data = pram;
      data.pageNo = index || 1;
      this.loading = true;
      queryRejectLease(data).then(res => {
        if (res.data.itemCount === 0) {
          this.loadAll = true;
        } else {
          this.pageIndex += 1;
          this.refreshing = false;
          this.loading = false;
          this.listData = this.listData.concat(res.data.checkouts);
          if (res.data.itemCount === this.listData.length) {
            this.loadAll = true;
          }
        }
      });
    }
  },
  created () {
    this.getListData(this.params);
  },
  watch: {
    params: {
      handler: function (newVal, oldVal) {
        if (this.time_out) {
          clearTimeout(this.time_out);
        }
        this.time_out = setTimeout(() => {
          this.listData = [];
          this.pageIndex = 1;
          this.refreshing = false;
          this.loadAll = false;
          this.load();
        }, 600);
      },
      deep: true
    }
  }
};
</script>
<style lang="less" scoped>
.topNav {
  .right_solt {
    display: flex;
    align-items: center;
  }
}
// 列表区
.v_mainContainer.skipRem {
  position: relative;
  height: calc(100% - 104px);
}
// tab 卡样式重写
/deep/.mu-tab {
  color: #797979;
  min-width: auto;
  &.mu-tab-active {
    color: #000000;
  }
}
// 搜索框样式调整
.search_wrap {
  display: flex;
  align-items: baseline;
  width: 100%;
  // 搜索框
  .cancle.skipRem {
    height: 30px;
  }
  .searchBox {
    flex: 1;
    // 搜索框调整
    &/deep/.mu-input {
      .mu-text-field {
        width: 100%;
        background: #ffffff;
        border-radius: 5px;
        padding-left: 10px;
        line-height: 100%;
        box-shadow: 0px 2px 2px 0px rgba(16, 16, 16, 0.3);
      }
    }
  }
}
</style>
