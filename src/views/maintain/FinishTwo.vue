<template>
    <div class="maintain">
            ...
            <div v-if="addStatus" class="addContent">
                <mu-form ref="form" :model="validateForm">
                    ...
                    <mu-form-item class="eachRow" prop="costTime" label-float label="产生时间" :rules="timeRules">
                        <mu-date-input container="bottomSheet" v-model="validateForm.costTime" label-float full-width no-display></mu-date-input>
                    </mu-form-item>
        ...
        <div class="bottom">
            ...
            <mu-button v-if="nextStatus" class="ml20" color="#fdd835" @click='next' textColor="#000" data-mu-loading-text="请稍后.." v-loading="submitLoading" :disabled="submitLoading">下一步</mu-button>
...
export default {
  ...
  methods: {
    ...
    // 完成
    finshOrder () {
      let param = JSON.parse(localStorage.getItem('finishInfo'));
      param.orderId = localStorage.getItem('orderId');
      param.checkResult = 1;
      check(param).then(res => {
        if (res.code === 200) {
          this.$router.push('/finishFour');
          this.submitLoading = false;
        }
      })
        .catch(errMsg => {
